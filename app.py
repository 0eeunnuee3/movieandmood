...
.hero-eyebrow {
    ...
    color: #e63946; margin-bottom: 14px;
}
.hero-title {
    font-family: 'Bebas Neue', sans-serif;
    font-size: 5.5rem; line-height: 0.9;
    color: #ffffff; margin: 0 0 16px 0; letter-spacing: 3px;
}
.hero-title span { color: #e63946; }
.hero-sub { font-size: 0.95rem; color: #888; letter-spacing: 1px; }
.film-strip-deco {
    position: absolute; right: 0; top: 0; bottom: 0; width: 60px;
    background: #111; display: flex; flex-direction: column;
    align-items: center; justify-content: space-around;
    padding: 8px 0; border-left: 1px solid #2a2a3a;
}
.film-hole {
    width: 16px; height: 12px; background: #0a0a0f;
...
.card:hover { background: #14141f; border-left-color: #ff6b6b; }
.card-num { font-family: 'Bebas Neue', sans-serif; font-size: 0.8rem; color: #e63946; letter-spacing: 2px; margin-bottom: 4px; }
.card-title { font-family: 'Bebas Neue', sans-serif; font-size: 1.4rem; color: #ffffff; letter-spacing: 1px; margin: 0 0 6px 0; }
...
.tag {
    display: inline-block; border: 1px solid #e63946; border-radius: 2px;
    font-size: 0.62rem; font-weight: 600; padding: 2px 8px; margin: 2px;
    color: #e63946; text-transform: uppercase; letter-spacing: 1.5px;
}
.tag.b { border-color: #4a9eff; color: #4a9eff; }
.tag.g { border-color: #4aff9e; color: #4aff9e; }
.tag.w { border-color: #aaa; color: #aaa; }

.sec-label { font-size: 0.62rem; font-weight: 600; letter-spacing: 4px; text-transform: uppercase; color: #e63946; margin-bottom: 4px; }
.sec-title { font-family: 'Bebas Neue', sans-serif; font-size: 2rem; color: #ffffff; letter-spacing: 2px; margin: 0 0 18px 0; border-bottom: 1px solid #2a2a3a; padding-bottom: 10px; }
.badge { display: inline-block; background: #e63946; color: #fff; font-size: 0.75rem; font-weight: 700; padding: 4px 14px; border-radius: 2px; margin-bottom: 16px; letter-spacing: 1px; text-transform: uppercase; }

.chart-card { background: #0f0f18; border: 1px solid #2a2a3a; border-radius: 4px; padding: 16px; margin-bottom: 14px; }
.chart-title { font-size: 0.65rem; font-weight: 700; letter-spacing: 3px; text-transform: uppercase; color: #555; margin-bottom: 10px; }
...
.random-card::after { content: '★'; position: absolute; right: 24px; top: 50%; transform: translateY(-50%); font-size: 6rem; color: #e63946; opacity: 0.08; }
.random-eyebrow { font-size: 0.65rem; letter-spacing: 4px; text-transform: uppercase; color: #e63946; margin-bottom: 8px; }
.random-title { font-family: 'Bebas Neue', sans-serif; font-size: 2.4rem; color: #fff; letter-spacing: 2px; margin: 4px 0 8px 0; }
...
div[data-testid="stButton"] button:hover { background: #ff6b6b !important; }
...
with st.sidebar:
    st.markdown("### 🎭 MOOD")
    selected_moods = st.multiselect("기분 선택 (여러 개 가능)", options=list(MOOD_LABELS.keys()), default=["comforting"], format_func=lambda x: MOOD_LABELS[x])
    st.markdown("---")
    st.markdown("### 🎬 GENRE")
    selected_genres = st.multiselect("장르 필터", options=ALL_GENRES)
    st.markdown("---")
    st.markdown("### 📅 YEAR")
    year_range = st.select_slider("", options=["~1990","1990s","2000s","2010s","2020s~","전체"], value="전체")
    st.markdown("---")
    st.markdown("### ⏱️ RUNTIME")
    runtime_option = st.select_slider(" ", options=["90분 이하","90~120분","120분 이상","상관없음"], value="상관없음")
    st.markdown("---")
...
with col_l:
    ...
    if top20.empty:
        ...
        for i, (_, row) in enumerate(top20.iterrows()):
            ...
            tags_html = "".join(f'<span class="tag {TAG_COLORS[j%len(TAG_COLORS)]}">{e}</span>' for j,e in enumerate(row["emotions"]))
            st.markdown(f"""
            <div class="card">
...
with col_r:
    ...
    BG="rgba(0,0,0,0)"; FC="#888"; MG=dict(l=0,r=0,t=6,b=0)
    def chart_layout(fig, xt, yt=""):
        fig.update_layout(plot_bgcolor=BG, paper_bgcolor=BG, font_color=FC,
                          showlegend=False, coloraxis_showscale=False, margin=MG,
    ...
    if all_g:
        ...
        fig1 = px.bar(x=gc.values, y=gc.index, orientation="h", color=gc.values, color_continuous_scale=["#2a1a1a","#e63946"])
    ...
    fig3 = px.histogram(top20.dropna(subset=["release_year"]), x="release_year", nbins=15, color_discrete_sequence=["#4a9eff"])
